...
- ServiceImpl 层依赖仓储端口接口，MyBatis/内存仓储等实现没有向上穿透。
- 写接口从 Token 获取当前用户身份。
- 权限、归属、可见范围由后端校验。
...
前后端联调阶段，后端子代理默认作为后端观察子代理参与。

必须检查并反馈：

- `integration-plan.md` 中后端接口、入参、响应、错误码是否与实际 DTO、Controller 和服务实现一致。
- 后端服务边界、数据库归属、profile、端口和中间件依赖是否满足本次联调需要。
- 接口是否具备必要参数校验、认证身份来源、权限判断和统一异常响应。
- 是否能提供 `requestId`、`traceId`、`spanId`、`serviceName`、HTTP 状态和业务错误码等日志证据。
- 是否记录服务启动证据、健康检查结果、脱敏请求响应、数据库摘要和接口访问摘要。
- `issue-log.md` 中后端日志摘要是否脱敏，不能记录 Token、密码、密钥、完整手机号、完整邮箱、私密推荐正文或评论正文。

后端观察子代理默认不修改代码。如需修复后端问题，必须由主代理重新授权写入范围。
