...
FoodMap 默认按可联调业务切片推进。每完成一个前端或后端功能，主代理必须判断该功能是否已经到达联调安全点，并主动说明：

- 当前功能是否建议先联调。
- 如果只完成了后端，前端需要开发到什么最小程度才能联调。
- 如果只完成了前端，后端需要提供哪些接口、数据和日志能力才能联调。
- 如果继续开发下一个功能而不联调，主要风险是什么。

强依赖链路，如登录/Token、门店地图查询、推荐创建、可见范围、评论权限、PUBLIC 统计和图片上传，达到联调安全点后应优先联调，再进入下一个依赖它的功能。
