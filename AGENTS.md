...
```text
前端子代理：
只允许修改 ios/FoodMapApp/Features/Auth 和 ios/FoodMapApp/Core/Networking。

后端认证子代理：
只允许修改 backend/foodmap-auth-service 和 backend/foodmap-common。

后端门店子代理：
只允许修改 backend/foodmap-store-service。
...
harness 当前是轻量阶段。后端和 iOS 工程尚未生成时，脚本会跳过对应代码检查；等工程落地后再逐步增强。
